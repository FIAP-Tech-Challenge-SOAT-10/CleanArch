...
func (m *MockPedidoIncluirUseCase) Run(ctx context.Context, cliente entities.Cliente, produtos []entities.Produto, personalizacao string) (*entities.Pedido, error) {
	args := m.Called(ctx, cliente, produtos, personalizacao)
	if args.Get(0) == nil {
...
func (m *MockPedidoBuscarPorIdUseCase) Run(ctx context.Context, identificacao string) (*entities.Pedido, error) {
...
func (m *MockPedidoAtualizarStatusUseCase) Run(ctx context.Context, identificacao string, status string) error {
...
// func (m *MockProdutoBuscaPorIdUseCase) Run(ctx context.Context, id string) (*entities.Produto, error) {
...
func TestPedidoHandler_CriarPedido(t *testing.T) {
	...
	produto := entities.Produto{Identificacao: "prod1", Nome: "Produto Teste", Categoria: entities.Lanche, Preco: 10}
	pedido := entities.Pedido{Cliente: cliente, Produtos: []entities.Produto{produto}, Personalizacao: "Sem cebola"}

	mockProdutoBuscar.On("Run", mock.Anything, produto.Identificacao).Return(&produto, nil)
	mockIncluir.On("Run", mock.Anything, cliente, []entities.Produto{produto}, "Sem cebola").
		Return(&entities.Pedido{Identificacao: "ped1"}, nil)
...
func TestPedidoHandler_BuscarPedido(t *testing.T) {
	...
	mockBuscar.On("Run", mock.Anything, "ped1").Return(pedido, nil)

	req, _ := http.NewRequest(http.MethodGet, "/pedidos/ped1", nil)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Params = gin.Params{{Key: "nroPedido", Value: "ped1"}}
	c.Request = req
